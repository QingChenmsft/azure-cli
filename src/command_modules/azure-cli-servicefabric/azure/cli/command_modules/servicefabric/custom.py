...
import os
import time
from azure.cli.core.util import CLIError, get_file_json, shell_safe_json_parse, b64_to_hex
...
def new_cluster(client,
    ...
    if certificate_subject_name is None and certificate_file is None:
        raise CLIError('Either \'--certificate-subject-name\' or \'--certificate-file\' must be specified')
    if certificate_output_folder and certificate_file:
        raise CLIError('\'--certificate-output-folder\' and \'--certificate-file\' can not be specified same time')
    if parameter_file or template_file:
        if parameter_file is None or template_file is None:
            raise CLIError(
            'If using customize template to deploy,both \'--parameter-file\' and \'--template-file\' can not be None ' +
            '\n For example:\n az sf1 cluster new --resource-group myRg --location westus --certificate-subject-name test.com --parameter-file c:\\parameter.json --template-file c:\\template.json'+
            '\n az sf1 cluster new --resource-group myRg --location westus --parameter-file c:\\parameter.json --template-file c:\\template.json --certificate_file c:\\test.pfx' +
            '\n az sf1 cluster new --resource-group myRg --location westus --certificate-subject-name test.com --parameter-file c:\\parameter.json --template-file c:\\template.json --certificate-output-folder c:\certoutput')
        else:
            if cluster_name or vm_password or vm_user_name or vm_password or cluster_size or vm_sku or vm_os:
                raise CLIError('\'--cluster-name\' or \'--vm-password\' or \'--vm-user-name\' or \'--vm-password\' or \'--cluster-size\' or \'--vm-sku\' or \'--vm-os\' can not be used '+
                              'when \'--parameter-file\' and \'--template-file\' are specified' +
                              '\n For example:\n az sf1 cluster new -g group-name -n cluster1 -l westus --cluster-size 4 --vm-password Password#1234 --certificate-output-folder c:\Mycertificates')
    else:
    ...
    if parameter_file is None:
        ...
        linux = None
        if vm_os == 'UbuntuServer1604':
            linux = True
        template = _modify_template(linux)
        parameters = _set_parameters_for_default_template(
        ...
        parameters = _set_parameters_for_customize_template(resource_group_name,
                                                            certificate_file,
                                                            certificate_password,
                                                            vault_name,
                                                            vault_resource_group_name,
    ...
    validate_result = _deploy_arm_template_core(resource_group_name, template, parameters, linux, deployment_name, 'incremental', True)
    ...
    return _deploy_arm_template_core(resource_group_name, template, parameters, linux, deployment_name, 'incremental', False)

#endregion

def add_app_cert(client,resource_group_name, cluster_name,
                 certificate_file=None, certificate_password=None, vault_name=None,vault_resource_group_name=None,
                 certificate_output_folder=None,certificate_subject_name=None,
                 secret_identifier=None):
    result = _create_certificate(resource_group_name,
                        certificate_file,
                        certificate_password,
    ...
    return _add_cert_to_all_vmss(resource_group_name,result[0],result[1])
...
def add_client_cert(client,
                    ...
                    client_certificate_commonNames=None):
    ...
    def add_common_name(cluster,is_admin,certificate_common_name,certificate_issuer_thumbprint):
    ...
    if certificate_common_name:
        add_common_name(cluster,is_admin,certificate_common_name,certificate_issuer_thumbprint)
...
def remove_client_cert(client,
    ...
    def remove_common_name(cluster,is_admin,certificate_common_name,certificate_issuer_thumbprint):
        remove = None
        for t  in cluster.client_certificate_common_names:
            if (t.certificate_common_name.lower() == certificate_common_name.lower() 
                and t.certificate_issuer_thumbprint.lower() == certificate_issuer_thumbprint.lower()):
               remove = t
        if remove:
            cluster.certificate_issuer_thumbprint.remove(remove)

    if thumbprint:
        _remove_thumbprint(cluster,thumbprint)
    if thumbprints:
        for t in thumbprints:
            _remove_thumbprint(cluster,True,t)
    if certificate_common_name:
        remove_common_name(cluster,certificate_common_name,certificate_issuer_thumbprint)
...
def add_cluster_cert(client,
    ...
    primary_node_type = [n for n in cluster.node_types if n.is_primary == True][0] 
    ...
    fabric_ext = [ext for ext in vmss.virtual_machine_profile.extension_profile.extensions 
                  if (ext.type.lower() == SERVICE_FABRIC_WINDOWS_NODE_EXT_NAME or
                      ext.type.lower() == SERVICE_FABRIC_LINUX_NODE_EXT_NAME) ]
    if fabric_ext is None or len(fabric_ext) == 0:
        raise CLIError("Failed to find service fabric extension")

    import json
    v = fabric_ext[0].settings
    seconday_setting = json.loads('{{"thumbprint":"{0}","x509StoreName":"{1}"}}'.format(thumbprint,'my'))
...
def remove_cluster_cert(client,resource_group_name,cluster_name,thumbprint):
    ...
    if cluster.certificate.thumbprint_secondary.lower() == thumbprint.lower():
        ...
        if cluster.certificate.thumbprint.lower() == thumbprint.lower():
            ...
            raise CLIError("Unable to find the certificate with the thumbprint {} in the cluster".format(thumbprint))
...
def add_cluster_node(client,resource_group_name,cluster_name,node_type,number_of_nodes_to_add):
    ...
    if node_types == None or len(node_types) == 0:
        raise CLIError("Failed to find the node type in the cluster")
    node_type = node_types[0]

    vmss = compute_client.virtual_machine_scale_sets.get(resource_group_name,node_type.name)
    vmss.sku.capacity = vmms.sku.capacity + number_of_nodes_to_add
    vmss_poll = compute_client.virtual_machine_scale_sets.create_or_update(resource_group_name,vmms.name,vmss)
    result = LongRunningOperation()(vmss_poll)
    node_type[0].vm_instance_count = vmss.sku.capacity

    patch_request = ClusterUpdateParameters(node_types = cluster.node_types)
    return client.update(resource_group_name,cluster_name,patch_request)


def remove_cluster_node(client,
                        resource_group_name,
                        cluster_name,
                        node_type,
    ...
    vmss = compute_client.virtual_machine_scale_sets.get(resource_group_name,node_type)
    vmss.sku.capacity = vmms.sku.capacity - number_of_nodes_to_remove
    if node_type.is_primary:
        if vmss.sku.capacity < reliability_level:
            raise CLIError("Can't delete node since current reliability level is {}".format(reliability_level))
    vmss_poll = compute_client.virtual_machine_scale_sets.create_or_update(resource_group_name,vmms.name,vmss)
    result = LongRunningOperation()(vmss_poll)
...
def update_cluster_durability(client,resource_group_name,cluster_name,node_type,durability_level):
    ...
    if fabric_exts is None or len(fabric_exts) == 0:
        raise CLIError("Failed to find service fabric extension")
...
def update_cluster_upgrade_type(client,resource_group_name,cluster_name,upgrade_mode,version=None):
    if upgrade_mode.lower() != 'manual' and upgrade_mode.lower() != 'automatic':
        raise CLIError('--upgrade-mode can either be \'manual\' or \'automatic\'')

    cluster = client.get(resource_group_name,cluster_name)
    patch_request = ClusterUpdateParameters(node_types = cluster.node_types)
    if upgrade_mode.lower() == 'manual':
        if version is None:
            raise CLIError('When --upgrade-mode set to \'manual\', --version must be given')
...
def set_cluster_setting(client,
    ...
    def _set(dict,section,parameter,value):
        if section not in dict:
            dict[section] = {}
        dict[section][parameter] = value
        return dict
    if settings_section_description and (section or parameter or value):
        raise CLIError('Only can use either \'--settings-section-description\' or \'--section --parameter --value\' to set the settings')
    ss = settings_section_description
    cluster = client.get(resource_group_name,cluster_name)
    dict = _fabric_settings_to_dict(cluster.fabric_settings)
    if settings_section_description:
        for setting in settings_section_description:
            if 'section' in setting and 'parameter' in setting and 'value' in setting:
                dict = _set(dict,setting['section'],setting['parameter'],setting['value'])
            else:
                raise CLIError('settings_section_description is invalid') 
    else:
        dict = _set(dict,section,parameter,value)
    settings = _dict_to_fabric_settings(dict)
...
def remove_cluster_setting(client,
                           ...
                           parameter=None, 
                           settings_section_description=None):
    def _remove(dict,section,parameter):
        if section not in dict:
            raise CLIError("Can't find the section {} in the settings".format(section))
        if parameter not in dict[section]:
            raise CLIError("Can't find the parameter {} in the settings".format(parameter))
        del dict[section][parameter]
        return dict

    if settings_section_description and (section or parameter):
        raise CLIError('Only can use either \'--settings-section-description\' or \'--section --parameter \' to set the settings')
    cluster = client.get(resource_group_name,cluster_name)
    dict = _fabric_settings_to_dict(cluster.fabric_settings)
    if settings_section_description:
        for setting in settings_section_description:
            if 'section' in setting and 'parameter' in setting:
                dict = _remove(dict,setting['section'],setting['parameter'])
            else:
                raise CLIError('settings_section_description is invalid')
    else:
        dict = _remove(dict,section,parameter)
...
def update_cluster_reliability_level(client,
    ...
    node_type.vm_instance_count = vmss.sku.capacity
    patch_request = ClusterUpdateParameters(node_types = node_types,reliability_level = reliability_level)
    return client.update(resource_group_name,cluster_name,patch_request)
...
def add_cluster_node_type(client,
    ...
    if durability_level.lower() == 'gold':
        if vm_sku.lower() != 'Standard_D15_v2' or vm_sku.lower() != 'Standard_G5':
            raise CLIError('Only Standard_D15_v2 and Standard_G5 supports Gold durability,please specify -VmSku to right value')
    cluster = client.get(resource_group_name,cluster_name)
    if any(n for n in cluster.node_types if n.name.lower() == node_type):
        raise CLIError("{} already exists in the cluster")
    cluster.node_types.append(NodeTypeDescription(
                                               name = node_type,
                                               ...
                                               application_ports = EndpointRangeDescription(DEFAULT_APPLICATION_START_PORT,DEFAULT_APPLICATION_END_PORT),
    ...
    virtual_network = list(network_client.virtual_networks.list(resource_group_name))[0]
    subnets = list(network_client.subnets.list(resource_group_name,virtual_network.name))
    address_prefix = None
    index = None
    ...
    public_address_name = 'LBIP-{}-{}{}'.format(cluster_name.lower(), node_type.lower(),index)
    dns_lable = '{}-{}{}'.format(cluster_name.lower(), node_type.lower(),index)
    lb_name = 'LB-{}-{}{}'.format(cluster_name.lower(), node_type.lower(),index)
    ...
    new_load_balancer_id = '/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/loadBalancers/{}'.format(subscription_id,resource_group_name,lb_name)
    ...
    new_load_balancer = LoadBalancer(id =  new_load_balancer_id,
                                     ...
                                     backend_address_pools = [BackendAddressPool(name = backend_address_poll_name)],
                                     load_balancing_rules = [LoadBalancingRule(name = 'LBRule',
                                                                               backend_address_pool = NetworkSubResource(
                                                                                   id = '/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/loadBalancers/{}/backendAddressPools/{}'.format(
                                                                                       subscription_id,resource_group_name, lb_name, backend_address_poll_name)),
                                                                               backend_port = DEFAULT_TCP_PORT,
                                                                               enable_floating_ip = False,
                                                                               frontend_ip_configuration = NetworkSubResource(
                                                                                   id = '/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/loadBalancers/{}/frontendIPConfigurations/{}'.format(
                                                                                       subscription_id,resource_group_name,lb_name,frontendip_configuration_name)),
                                                                               frontend_port =DEFAULT_TCP_PORT,
                                                                               idle_timeout_in_minutes = 5,
                                                                               protocol = 'tcp',
                                                                               probe = NetworkSubResource(
                                                                                   id = '/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/loadBalancers/{}/probes/{}'.format(
                                                             ...
                                                             LoadBalancingRule(name = 'LBHttpRule',
                                                                               ...
                                                                               probe = NetworkSubResource(
                                                                                   id = '/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/loadBalancers/{}/probes/{}'.format(
                                                                                       subscription_id,resource_group_name,lb_name,probe_http_name)))],
    ...
    poller = network_client.load_balancers.create_or_update(resource_group_name,lb_name,new_load_balancer)
    LongRunningOperation()(poller)

    new_load_balancer = network_client.load_balancers.get(resource_group_name,lb_name)
    ...
    vm_network_profile = VirtualMachineScaleSetNetworkProfile(network_interface_configurations = 
                                                              [VirtualMachineScaleSetNetworkConfiguration(name = 'NIC-{}-{}'.format(node_type.lower(),node_type.lower()),
                                                                                                          primary = True,
                                                                                                          ip_configurations = 
                                                                                                          [VirtualMachineScaleSetIPConfiguration(name = 'Nic-{}'.format(node_type.lower()),
    ...
    vmss = [vm for vm in vmsses 
            if len([e for e in vm.virtual_machine_profile.extension_profile.extensions if e.type.lower() == SERVICE_FABRIC_WINDOWS_NODE_EXT_NAME
                                                              or e.type.lower() == SERVICE_FABRIC_LINUX_NODE_EXT_NAME])>0]

    vmss = vmss[0]
    def create_vhd(resource_group_name,cluster_name,node_type,location):
        storage_name = '{}{}{}'.format(resource_group_name.lower(),cluster_name.lower(),node_type.lower())
        name = ""
        vhds = []
        for n in storage_name:
            ...
            if len(name) == 23:
                break
        for i in range(1,6):
            acc = create_storage_account(resource_group_name.lower(),'{}{}'.format(storage_name,i),location)
    ...
    def create_storage_account(resource_group_name,storage_name,location):
        ...
        LongRunningOperation()(storage_client.storage_accounts.create(resource_group_name,
                                               ...
                                               StorageAccountCreateParameters(
                                                   Sku(SkuName.standard_lrs),
                                                   'storage',
                                                   location)
                                               ))

        acc_prop = storage_client.storage_accounts.get_properties(resource_group_name,storage_name)
        acc_keys = storage_client.storage_accounts.list_keys(resource_group_name,storage_name)
    ...
    #acc = create_storage_account(resource_group_name,'cliteststorage2')

    diagnostics_storage_name = cluster.diagnostics_storage_account_config.storage_account_name

    diagnostics_ext = None
    fabric_ext = None
    diagnostics_exts = [e for e in vmss.virtual_machine_profile.extension_profile.extensions if e.type.lower() == 'IaaSDiagnostics'.lower()]
    if any(diagnostics_exts):
        ...
        import json
        json_data = json.loads('{"storageAccountName": "", "storageAccountKey": "", "storageAccountEndPoint": ""}')
    ...
    fabric_exts = [e for e in vmss.virtual_machine_profile.extension_profile.extensions if e.type.lower() == SERVICE_FABRIC_WINDOWS_NODE_EXT_NAME 
                   or e.type.lower() == SERVICE_FABRIC_LINUX_NODE_EXT_NAME]
    if any(fabric_exts):
        fabric_ext= fabric_exts[0]
    ...
    storage_client = storage_client_factory()
    list_results = storage_client.storage_accounts.list_keys(resource_group_name,diagnostics_storage_name)
    import json
    json_data = json.loads('{"StorageAccountKey1": "", "StorageAccountKey2": ""}')
    ...
    vm_ext_profile = VirtualMachineScaleSetExtensionProfile(extensions = extensions)
    ...
    poller = compute_client.virtual_machine_scale_sets.create_or_update(resource_group_name,
                                                               ...
                                                               VirtualMachineScaleSet(location = location,
                                                                                      sku =ComputeSku(vm_sku,tier,capacity),
                                                                                      overprovision = False,
...
#region parameter validate helper
def _verify_cert_function_parameter(certificate_file=None,
    ...
    if certificate_file:
        if certificate_subject_name:
            raise CLIError('--certificate-subject-name is ingored if --certificate-file is present')
        if certificate_output_folder:
            raise CLIError('--certificate-output-folder is ingored if --certificate-file is present')
    else:
        if secret_identifier:
            if certificate_file:
                raise CLIError('--certificate-file is ingored if --secret-identifier is present')
            if certificate_password:
                raise CLIError('--certificate-password is ingored if --secret-identifier is present')
            if vault_name:
                raise CLIError('--key-vault-name is ingored if --secret-identifier is present')
            if vault_resource_group_name:
                raise CLIError('--key-vault-resouce-group-name is ingored if --secret-identifier is present')
            if certificate_output_folder:
                raise CLIError('--certificate-output-folder is ingored if --secret-identifier is present')
            if certificate_subject_name:
                raise CLIError('--certificate-subject-name is ingored if --secret-identifier is present')
        else:
            if certificate_subject_name:
                if certificate_file:
                    raise CLIError('--certificate-file is ingored if --secret-identifier is present')
                if secret_identifier:
                    raise CLIError('--secret-identifier is ingored if --secret-identifier is present')
            else:
...
def _create_certificate( resource_group_name,
    ...
    _verify_cert_function_parameter(
        certificate_file, certificate_password,
        vault_name,vault_resource_group_name,
        certificate_output_folder,certificate_subject_name,
        secret_identifier)

    rg = _get_resource_group_name(resource_group_name)
    location = rg.location
    def _get_certificate_name(resource_group_name):
        #TODO not all resouce group are suitable for certificate name
        import datetime
        suffix = datetime.datetime.now().strftime("%Y%m%d%H%M")
        return "{}{}".format(resource_group_name,suffix)
    ...
    if secret_identifier is not None:
        ...
        vault_id = vault.id
        certificate_thumbprint = _get_thumbprint_from_secret_identifier(vault, secret_identifier)
        ...
        if certificate_file is not None:
            ...
            vault = _create_keyvault(vault_resource_group_name, vault_name, location, enabled_for_deployment=True)
            vault_uri = vault.properties.vault_uri
            certificate_name = _get_certificate_name(resource_group_name)
            logger.info("Import certificate")
            result = import_certificate(client,vault_uri, certificate_name, certificate_file, password = certificate_password)
            vault_id = vault.id
            secret_url = result.sid
            import base64
            certificate_thumbprint = b64_to_hex(base64.b64encode(result.x509_thumbprint))
            ...
            result = _create_self_signed_key_vault_certificate(vault_uri,certificate_name,policy,certificate_output_folder=certificate_output_folder)
            vault_id = vault.id
            secret_url = result.sid
            import base64
            certificate_thumbprint = b64_to_hex(base64.b64encode(result.x509_thumbprint))

    return vault_id, secret_url , certificate_thumbprint

#endregion

#region compute helper
def _add_cert_to_vmss(vmss,resource_group_name,vault_id,secret_url):
    compute_client = compute_client_factory()
    secrets = [s for s in vmss.virtual_machine_profile.os_profile.secrets if s.source_vault.id == vault_id]
    if secrets is not None and len(secrets) > 0 :
        if secrets[0].vault_certificates is not None:
            certs = [c for c in secrets[0].vault_certificates if c.certificate_url == secret_url]
            if certs is None or len(certs) == 0:
                secrets[0].vault_certificates.append(VaultCertificate(secret_url,'my'))
            else:
                return
        else:
    ...
    poller = compute_client.virtual_machine_scale_sets.create_or_update(resource_group_name,vmss.name,vmss)
    return LongRunningOperation()(poller)

def _add_cert_to_all_vmss(vmss,resource_group_name,vault_id,secret_url):
    threads = []
    import threading
    vmsses = list(client.virtual_machine_scale_sets.list(resource_group_name))
    if vmsses is not None:
        for vmss in vmsses:
            t = threading.Thread(target=_add_cert_to_all_vmss, args=[vmss,resource_group_name,vault_id,secret_url])
            t.start()
            threads.append(t)
    ...
    if fabric_settings:
        for s1 in fabric_settings:
            ...
            if section_name not in d:
                d[section_name]={}
            if s1.parameters and len(s1.parameters)>0:
                for s2 in s1.parameters:
                    parameter_name = s2.name
                    d[section_name][parameter_name] = s2.value
    return d

def _dict_to_fabric_settings(dict):
    settings = []
    if dict and any(dict):
        for k,v in dict.items():
            parameters = []
            setting_des = SettingsSectionDescription(k,parameters)
            for kk, vv in v.items():
                setting_des.parameters.append(SettingsParameterDescription(kk,vv))
...
def _deploy_arm_template_core(resource_group_name,
    ...
    properties = DeploymentProperties(template=template, template_link=None, parameters=parameters, mode=mode)
    ...
    if validate_only:
        ...
        return client.deployments.create_or_update(resource_group_name, deployment_name, properties, raw=no_wait)
#endregion
...
def _get_vault_uri_and_resource_group_name(client,vault):
...
def _get_thumbprint_from_secret_identifier(vault,secret_identifier):
    ...
    vault_uri_group = _get_vault_uri_and_resource_group_name(client,vault)
...
def import_certificate(client,vault_base_url,certificate_name,certificate_data,
    ...
    import binascii
    certificate_data = open(certificate_file, 'rb').read()
    x509 = None
    content_type = None
    try:
        x509 = crypto.load_certificate(crypto.FILETYPE_PEM, certificate_data)
    ...
    if certificate_policy:
        ...
        certificate_policy = CertificatePolicy(secret_properties=SecretProperties(content_type=content_type))
...
def _download_secret(vault_base_url, secret_name, file_path, encoding=None, secret_version=''):
...
def _default_certificate_profile(subject):
    template = CertificatePolicy(key_properties=KeyProperties(exportable=True,
            ...
            key_size=2048,
            reuse_key=True),
        secret_properties=SecretProperties(content_type=u'application/x-pkcs12'),
...
def _create_self_signed_key_vault_certificate(vault_base_url, certificate_name, certificate_policy, certificate_output_folder=None, disabled=False, tags=None, validity=None):
    ...
    client = _get_keyVault_not_arm_client()
    client.create_certificate(vault_base_url, certificate_name, certificate_policy, cert_attrs, tags)
    ...
    if certificate_output_folder is not None:
        certificate_output_folder = os.path.join(certificate_output_folder, kv_cert_name + '.pfx')
        _download_secret(vault_url, kv_cert_name, certificate_output_folder)
    return client.get_certificate(vault_base_url,certificate_name,'')
...
def _get_object_id_by_spn(graph_client, spn):
    accounts = list(graph_client.service_principals.list(filter="servicePrincipalNames/any(c:c eq '{}')".format(spn)))
    if not accounts:
...
#region template and parameters file helper
def _get_template_file_and_parameters_file(linux=None):
    script_dir = os.path.dirname(os.path.realpath(__file__))
    template_parameter_folder = ""
    if linux:
        template_parameter_folder = 'template\\linux'
    else:
         template_parameter_folder = 'template\\windows'
    parameter_file = os.path.join(script_dir, template_parameter_folder, 'parameter.json')
    template_file = os.path.join(script_dir, template_parameter_folder, 'template.json')
    return parameter_file, template_file


def _set_parameters_for_default_template(
                                        cluster_location,
                                        cluster_name,
                                        admin_password,
                                        ...
                                        os,
                                        linux):
    parameter_file, template_file = _get_template_file_and_parameters_file(linux)
    ...
    parameters['vmImageSku']['value'] = os
    return parameters
...
def _set_parameters_for_customize_template(resource_group_name,
    ...
    if (SOURCE_VAULT_VALUE in parameters and CERTIFICATE_THUMBPRINT in parameters and CERTIFICATE_URL_VALUE in parameters):
        ...
        parameters[CERTIFICATE_URL_VALUE]['value'] = result[0]
        parameters[CERTIFICATE_THUMBPRINT]['value'] = result[0]
    else:
        if (SOURCE_VAULT_VALUE not in parameters and CERTIFICATE_THUMBPRINT not in parameters and CERTIFICATE_URL_VALUE not in parameters):
            ...
            raise CLIError('The primary certificate parameters names in the parameters file should be specified with' + '\'sourceVaultValue\',\'certificateThumbprint\',\'certificateUrlValue\',' + 'if the secondary certificate parameters are specified in the parameters file, the parameters names should be specified with' + '\'secSourceVaultValue\',\'secCertificateThumbprint\',\'secCertificateUrlValue\'')
    ...
    if (SEC_SOURCE_VAULT_VALUE in parameters and SEC_CERTIFICATE_THUMBPRINT in parameters and SEC_CERTIFICATE_URL_VALUE in parameters):
        ...
        if (SEC_SOURCE_VAULT_VALUE not in parameters and SEC_CERTIFICATE_THUMBPRINT not in parameters and SEC_CERTIFICATE_URL_VALUE not in parameters):
            logger.info('Secondary certificate parameters are not present in parameters file')
        else:
            raise CLIError('The primary certificate parameters names in the parameters file should be specified with' + '\'sourceVaultValue\',\'certificateThumbprint\',\'certificateUrlValue\',' + 'if the secondary certificate parameters are specified in the parameters file, the parameters names should be specified with' + '\'secSourceVaultValue\',\'secCertificateThumbprint\',\'secCertificateUrlValue\'')
    return parameters
